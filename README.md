...
面向海外技术类网站的自动内容发布工作流。每 6 小时自动从 6 大英文技术主题中随机选题，由 DeepSeek 生成一篇 SEO/GEO 优化的原创英文技术文章，自动创建/复用分类与标签，查重后发布到 WordPress。**内容语言已切换为英文。**

### 覆盖主题

| 主题 | 说明 |
| --- | --- |
| Open Source Tutorials | 知名开源程序的安装、配置、使用教程 |
| Hosting Reviews | 主机/VPS 选购指南与测评（强调真实、不虚构数据） |
| Server Administration | Linux 命令、巡检、安全加固、备份、日志等实战 |
| WordPress Optimization | 性能、缓存、SEO、安全、插件优化教程 |
| Online Tools Tutorials | 实用在线工具的功能与用法 |
| Python/PHP Basics | 环境搭建、基础语法、常用库/框架、小实战 |
...
- **AI 生成**：DeepSeek 撰写原创英文技术文章，做 SEO/GEO 优化，输出结构化 JSON。
- **内容真实性约束**：提示词强制“不得编造版本号/价格/跑分/配置、以官方文档/官网为准”。
- **自动分类与标签**：分类/标签不存在时自动创建，存在时复用；标签不足时自动补齐默认标签（Tech Tutorials、Practical Tips、Web Hosting），保证标签始终 ≥ 3 个。
